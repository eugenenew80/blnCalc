...
public interface PowerTransformerModeRepo extends JpaRepository<PowerTransformerMode, Long> {
    @Query("select t from PowerTransformerMode t where t.meteringPoint.id = ?1 and ( (t.startDate is null or t.startDate <= ?2) and (t.endDate is null or t.endDate > ?3) or t.startDate between ?2 and ?3 or t.endDate between ?2 and ?3) order by t.startDate, t.endDate" )
