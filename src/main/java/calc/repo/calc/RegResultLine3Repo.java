...
public interface RegResultLine3Repo extends JpaRepository<RegResultLine3, Long> {
    ...
    @Query("select t from RegResultLine3 t where t.header.id = ?1 and t.meteringPoint.code = ?2 and t.val is not null")
    List<RegResultLine3> findByMeteringPoint(Long headerId, String mpCode);

    @Query("select t from RegResultLine3 t where t.header.id = ?1 and t.meteringPoint.code = ?2 and t.param = ?3 and t.val is not null")
